...
# ./config_files/debian_gnome_postinstall.sh
#
# This script assumes Debian was installed WITH the desktop task selected
# (GNOME + gdm3 already present). It replaces gdm3 with ly and then installs
# the same qtile setup as debian_postinstall.sh, skipping packages the
# desktop task already provides (xorg, NetworkManager, PipeWire audio). The
# GNOME session stays available as one of the options ly offers at login.



####################
# Replace gdm3 with ly
#

sudo systemctl disable --now gdm3 2>/dev/null
sudo apt purge -y gdm3 2>/dev/null || true
...
sudo apt install -y qtile picom dunst gpg curl wget git \
	...
	libabsl-dev brightnessctl lua5.4 luarocks \
	golang ripgrep xclip xsel bc rfkill \
